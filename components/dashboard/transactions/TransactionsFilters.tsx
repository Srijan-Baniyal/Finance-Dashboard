...
function getTypeTriggerClass(type: TransactionFilters["type"]): string {
  if (type === "income") {
    return "h-9 w-full rounded-lg border-emerald-300 bg-emerald-500/8 text-emerald-700 text-sm shadow-sm dark:border-emerald-700 dark:text-emerald-400";
  }
  if (type === "expense") {
    return "h-9 w-full rounded-lg border-rose-300 bg-rose-500/8 text-rose-700 text-sm shadow-sm dark:border-rose-700 dark:text-rose-400";
  }
  return "h-9 w-full rounded-lg border-border/70 bg-card text-sm shadow-sm";
}
...
export function TransactionsFilters({
  ...
  return (
    <Card className="overflow-hidden rounded-2xl border border-border/60 bg-card/80 shadow-sm">
      ...
      <CardContent className="space-y-3 bg-card/95 p-4 sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1">
            ...
            <Input
              className="h-9 w-full rounded-lg border-border/70 bg-card pr-9 pl-9 text-sm shadow-sm"
          ...
          <div className="grid grid-cols-2 gap-3 sm:w-96">
            ...
            >
              <SelectTrigger
                ...
                className={`h-9 w-full rounded-lg border-border/70 bg-card text-sm shadow-sm ${
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <Button
            className="hidden h-9 w-auto gap-1.5 rounded-lg border-border/70 bg-card text-muted-foreground shadow-sm hover:bg-muted sm:flex"
          ...
          <Button
            className="h-9 w-full gap-1.5 rounded-lg border-border/70 bg-card text-muted-foreground shadow-sm hover:bg-muted sm:hidden"
            disabled={activeCount === 0}
            onClick={resetFilters}
            type="button"
            variant="outline"
        ...
        <p className="text-[11px] text-muted-foreground">
          Date range auto-corrects to keep "From" and "To" valid.
        </p>
      </CardContent>
    </Card>
  );
